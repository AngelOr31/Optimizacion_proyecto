...
def bounding_phase(f, valor_i, delta=0.05, lm=-5, lM=5):
    """
    ...
    delta : float, opcional
        El incremento de búsqueda. Este valor define el tamaño del paso durante la búsqueda del mínimo. El valor predeterminado es 0.05.
    lm : float, opcional
        El límite inferior del intervalo de búsqueda. Define el rango inferior dentro del cual se busca el mínimo. El valor predeterminado es -5.
    lM : float, opcional
        El límite superior del intervalo de búsqueda. Define el rango superior dentro del cual se busca el mínimo. El valor predeterminado es 5.
    ...
    """
    k = 0
    x = valor_i[0] + 0.35  # Inicializa x con un valor ligeramente desplazado del valor inicial
    x_list = []
    y_list = []
...
def execute_total(fx, lm, lM, delta):
    ...
    plt.xlabel('x')
    plt.ylabel('y')
    plt.title("Resultado de la Optimización")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()